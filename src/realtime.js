...
export default class SocketController {
  constructor() {
    ...
    const config = window.__wooclap.clientConfig;
    ...
    this.encryptionKey = config.ablyEncryptionKey;
    this.encryptionKeyLength = config.ablyEncryptionKeyLength;
    this.encryptionAlgorithm = config.ablyEncryptionAlgorithm;
    this.encryptionMode = config.ablyEncryptionMode;
  }
  ...
  unsubscribeFromEvent(eventId, model) {
    if (!eventId) {
      throw new Error(`No event id for ${model.modelName}-${model.getId()}`);
    }
